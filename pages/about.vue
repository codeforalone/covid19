<template>
  <div class="About">
    ...
    <TextCard :title="$t('免責事項')">
      {{
        $t(
          '当サイトに掲載されている情報の正確性については万全を期していますが、本サイトは愛知県とは無関係の非公式のサイトです。'
        )
      }}<br />
      <br />
      {{
    ...
    <TextCard :title="$t('データについて')">
      <i18n
        tag="p"
        path="本サイトで公表しているデータは、愛知県のサイトより{catalogWebsite}のデータを加工したものです。"
      >
        <a
    ...
    <TextCard :title="$t('お問い合わせ先')">
      ...
      {{ $t('E-mail') }}:
      <a href="mailto:codeforalone@gmail.com">codeforalone@gmail.com</a>
    </TextCard>
  </div>
</template>
